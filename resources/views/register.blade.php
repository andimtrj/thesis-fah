<x-master>
    @if (session('error'))
        <div class="fixed top-4 right-4 w-11/12 sm:w-1/2 md:w-2/5 lg:w-2/5 bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded-md mb-4 z-50" role="alert">
            <strong class="font-bold">Registration Failed!</strong>
            <span class="block sm:inline">{{ session('error') }}</span>
            <button type="button" class="absolute top-2 right-2 p-1.5 text-yellow-500 hover:text-yellow-700" aria-label="Close" onclick="this.parentElement.style.display='none';">
                <svg class="h-4 w-4 fill-current" viewBox="0 0 20 20"><path d="M14.707 15.293a1 1 0 0 0 0-1.414L10.414 10l4.293-4.293a1 1 0 0 0-1.414-1.414L9 8.586 4.707 4.293A1 1 0 0 0 3.293 5.707L7.586 10l-4.293 4.293a1 1 0 0 0 1.414 1.414L9 11.414l4.293 4.293a1 1 0 0 0 1.414 0z"/></svg>
            </button>
        </div>
    @endif
      ...
      <div class="w-full bg-white shadow-2xl flex flex-col justify-center items-center">
        <form class="w-[32vw]" action="/registration" method="POST">
          ...
          <div id="formStep1">
            ...
            <div class="grid md:grid-cols-2 md:gap-6">
              <x-input type="tel" name="phoneNumber" id="phoneNumber" label="Phone number" value="{{ old('phoneNumber') }}" />
            ...
            <div class="w-full flex justify-end">
              <button id="nextButton" class="text-white bg-primary hover:bg-secondary font-medium rounded-2xl w-full text-md px-5 py-2.5 text-center mb-5">Next</button>
            </div>

            <p class="text-center text-gray-500">Already have an account? <a href="/login" class="underline text-blue-500">Login here</a></p>
          </div>
          {{-- End of First Form --}}
          ...
          <div id="formStep2" class="hidden">
            ...
            <div class="w-full flex justify-between">
              <button id="backButton" class="text-white bg-primary hover:bg-secondary font-medium rounded-2xl w-28 text-md px-5 py-2.5 text-center mb-5">Back</button>
              <button type="submit" class="text-white bg-primary hover:bg-secondary font-medium rounded-2xl w-28 text-md px-5 py-2.5 text-center mb-5">Submit</button>
            </div>
          </div>
          {{-- End of Second Form --}}
