  ...
  <table class="min-w-full leading-normal">
    <thead class="bg-cream">
      <tr>
        ...
        <th scope="col" class="px-4 py-3 text-center">Branch Admins</th>
      ...
      @if (isset($branches) && $branches->isNotEmpty())
        @foreach ($branches as $branch)
          <tr class="bg-white border-y text-base text-abu">
            ...
            <td class="px-3 text-center py-[10px] w-[10vw]">
              <a href="{{ route('branch-admin', ['branchId' => $branch->id]) }}" class="flex items-center justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" class="w-9 h-9 border-2 p-1 rounded-lg cursor-pointer">
            ...
            <td class="px-3 py-[10px] flex gap-4 items-center justify-center">
              <a href="{{ route('edit-branch', ['id' => $branch->id]) }}"
                class="border-2 w-fit p-1 rounded-lg cursor-pointer">
              ...
              <a data-modal-target="deleteModal" data-modal-toggle="deleteModal" class="border-2 w-fit p-1 rounded-lg cursor-pointer"
                href="" onclick="confirmation(event, {{ $branch->id }})">
