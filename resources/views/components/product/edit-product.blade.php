        ...
        <form action="{{ route('update-product', ['id' => $product->id]) }}" method="POST">
          ...
          <div class="mb-5">
            ...
            <input type="text" name="productName" id="productName"
              ...
              placeholder="Current Product name" required=""
                ...
                required>
                    @foreach ($branches as $branch)
                        <option value="{{ $branch->branch_code }}"
                            {{ $product->branch->branch_code == $branch->branch_code ? 'selected' : '' }}>
                            {{ $branch->branch_name }}
                        </option>
                    @endforeach
              </select>
          ...
          <div class="grid md:grid-cols-2 md:gap-6 mb-5 items-center">
            <div>
              ...
              <select id="productCategoryCode" name="productCategoryCode"
                ...
                @foreach ($productCategories as $productCategory)
                    <option value="{{ $productCategory->prod_category_code }}"
                        {{ $product->ProductCategory->prod_category_code == $productCategory->prod_category_code ? 'selected' : '' }}>
                        {{ $productCategory->prod_category_name }}
                    </option>
                @endforeach
            ...
            <div>
              ...
              <input type="number" id="productPrice" name="productPrice" aria-describedby="helper-text-explanation"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary block w-full p-2.5"
                placeholder="Type a number" step="1000" required value="{{ $product->product_price }}"/>
              @if ($errors->has('branchCode'))
                <p class="text-red-500 text-sm">{{ $errors->first('branchCode') }}</p>
              @endif
            </div>
