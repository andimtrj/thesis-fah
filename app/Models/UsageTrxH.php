...
class UsageTrxH extends Model
{
    ...
    public static function GetPagingUsageTrx(Request $request){
        $authTenantId = Auth::user()->tenant_id;
        $authBranchId = Auth::user()->branch_id;

        if ($authTenantId) {
            $query = UsageTrxH::join('branches as b', 'usage_trx_h.branch_id', '=', 'b.id')
                        ->join('users as u', 'usage_trx_h.user_create_id', '=', 'u.id')
                        ->join('roles as r', 'u.role_id', '=', 'r.id')
                        ->join('usage_trx_d as utd', 'utd.usage_trx_h_id', '=', 'usage_trx_h.id')
                        ->where('usage_trx_h.tenant_id', '=', $authTenantId);
            ...
            {
                $query->where('usage_trx_h.branch_id', '=', $authBranchId);
            }
            else
            {
